...
public class RegistrationActivity extends AppCompatActivity {
    ...
    private void saveinfo() {
        ...
        else if(!userphone.matches("\\d{10}"))
        {
            Toast.makeText(this, "phone no should be a 10 digit number", Toast.LENGTH_SHORT).show();
        }
        ...
        protected Void doInBackground(Void... voids) {
            try {
                ...
                new Handler(Looper.getMainLooper()).post(new Runnable() {
                    @Override
                    public void run() {
                        if(msg.equals("Registration is successfull"))
                        {
